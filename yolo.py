...
class YOLO(object):
    _defaults = {
        ...
        "classes_path": '/home/bhap/Pytorch_test/YoloV3/model_data/classescoco.txt',
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            batch_detections = non_max_suppression(output, self.config["yolo"]["classes"],
                                                   ...
                                                   nms_thres=0.3)
            # batch_detections = matrix_nms(output, self.config["yolo"]["classes"],
            #                               conf_thres=self.confidence,
            #                               nms_thres=0.4)
        try:
    ...
    # deepsort所用检测函数，不输出最终图像，输出检测框和类别
    def deepsort_detect_image(self, image):
        image_shape = np.array(np.shape(image)[0:2])
